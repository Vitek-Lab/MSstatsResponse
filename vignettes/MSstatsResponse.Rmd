...
# Data load / subset 
dia_pl = readRDS("../../../Chemoproteomics/Chemoproteomics_Pfizer/Pfizer/data/processed_data_MSstats/dia_proteinlevel.RDS")
...
# test nM to M dose 
dia_normalized$ProteinLevelData$dose = dia_normalized$ProteinLevelData$dose * 10^-9
```
...
```{r}
head(dia_pl)
dia_pl$drug = 'dasatinib' 
dia_pl = MSstatsPrepareDoseResponseFit(dia_pl,
...
#-log10(ic50_predictions$IC50 * 10^-9)
#-log10(ic50_predictions$IC50_lower_bound * 10^-9)
#-log10(ic50_predictions$IC50_upper_bound * 10^-9)


PredictIC50(dia_pl,
                               ratio_response = TRUE,
                               transform_dose = TRUE)
PredictIC50Parallel(dia_pl,
                        ratio_response = TRUE,
                        transform_dose = TRUE)

ic50_test_res = PredictIC50(dia_pl,
            n_samples = 1000,
            alpha = 0.10,
            increasing = FALSE,
            transform_dose = TRUE,
...
```{r}
prot_temp = 'p_strong_interaction_1'
sim_ic50_ratio %>%
  filter(protein == prot_temp)
sim_ic50_log %>%
  filter(protein == prot_temp)

protien_list = sim_ic50_ratio$protein[c(1:10,1001:1010)]

for (i in seq_along(protien_list)){
  
  prot_temp = protien_list[i]
  
  suppressWarnings({
  
    ratio_temp = VisualizeResponseProtein(sim_data,
                         drug_name = 'drug',
                         protein_name = prot_temp,
                         ratio_response = TRUE,
                         show_ic50 = TRUE,
                         add_ci = TRUE)
    print(ratio_temp)
    log_temp = VisualizeResponseProtein(sim_data,
                         drug_name = 'drug',
                         protein_name = prot_temp,
                         ratio_response = FALSE,
                         show_ic50 = TRUE,
...
Future Experiment  planing 
```{r}
setwd("../")
#c(0, 1, 3, 10, 30, 100, 300, 1000, 3000)
FutureExperimentSimulation(N_proteins = 3000, 
                           N_rep = 3, 
                           N_Control_Rep = 3, 
