...
```{r simulation_example, eval=TRUE}
...
The simulation generates three protein categories:  

- **Strong interactions**: Clear dose-response relationship  

- **Weak interactions**: Subtle dose-response relationship    

- **No interaction**: No dose-dependent change  


Performance metrics:  

- **TPR (True Positive Rate)**: Proportion of true interactions detected  

- **FPR (False Positive Rate)**: Proportion of non-interactions falsely detected  
...
- **Number of doses**: More doses improve curve resolution  

- **Dose spacing**: Log-scale spacing captures full dynamic range  

- **Replicates**: More replicates reduce variance and improve power  

- **Control replicates**: Extra DMSO replicates improve baseline estimation  


 

### Optimizing Dose Points and Replicates

The `PlotExperimentPowerCurve()` function helps visualize how detection power changes with different numbers of dose points and replicates:

```{r power_curves, fig.height=5, fig.width=8, eval=FALSE}
...
1. **Scale choice**:
   - Use **log2 scale** for interaction testing (variance stabilization)
   - Use **ratio scale** for IC50 estimation (interpretability)

2. **Multiple testing**:
   - Always use FDR-adjusted p-values for significance calls
   - Optional to loosen thresholds for exploratory analyses

3. **Bootstrap parameters**:
   - Use ≥1000 samples for stable confidence intervals
   - Increase samples if computational resources allow
