...
library(MSstats)
raw_data <- data.table::fread("~/Downloads/Selevsek2015-MSstats-input-90nodup-i.csv")
annotation <- data.table::fread("~/Downloads/Selevsek2015_DIA_Skyline_all_annotation.csv")
# raw_data <- read_tsv("path/to/spectronaut_report.tsv")

# Convert to MSstats format
msstats_data <- SkylinetoMSstatsFormat(raw_data, annotation)
...
Users can either use pre-processed data as shown above (e.g., dia_normalized\$ProteinLevelData) or begin at this step with their own datasets.
It is important to ensure that column names are correctly matched to the expected ones when using MSstatsPrepareDoseResponseFit().

The dose_column corresponds to the values on the x-axis (e.g., dose, concentration, time, or temperature).
The protein_column should contain unique identifiers (e.g., protein, gene, or peptide sequence), and the log_abundance_column represents the response values on the y-axis (e.g., log intensity, expression, or growth).
...
For this example, we have drug inhibitor dose-response data.
When treated with inhibitors we typically expect:
...
1.
Fits isotonic regression for each protein-drug pair\
2.
Performs F-test comparing the fitted model to a flat (null) model\
3.
Adjusts p-values for multiple testing using Benjamini-Hochberg procedure

```{r fit_dose_response, message=FALSE, warning=FALSE}
# Detect drug-protein interactions
response_results <- doseResponseFit(
  data = dia_prepared,
  increasing = TRUE,        # Expect decreasing response
...
n_significant <- sum(response_results$adjust_pval < 0.05, na.rm = TRUE)
...
# Confidence intervals don't seem to contain IC50 estimate??
...
ic50_predictions <- predictIC50(
  ...
  transform_dose = FALSE,     # Log-transform doses
...
# technically it's EC50 for increasing.... Terminology is confusing here.
ic50_predictions_increasing <- predictIC50(
  data = dia_prepared,
  ratio_response = TRUE,     # Use ratio scale for IC50
  transform_dose = FALSE,     # Log-transform doses
  increasing = TRUE,        # Decreasing response expected
  bootstrap = TRUE,          # Compute confidence intervals
  n_samples = 1000,         # Number of bootstrap samples
  alpha = 0.10,              # 90% confidence intervals,
  target_response = 1.5      # For increasing response
)
...
# Visualize strong responder (increasing)
visualizeResponseProtein(
  data = dia_prepared,
  protein_name = "YLL026W",
  drug_name = "T",
  ratio_response = TRUE,
  show_ic50 = TRUE,
  add_ci = TRUE,
...
Will probably need more explanation on how this works under the hood....

### Using your own data as templates

You can use proteins from your own experimental data as templates for more realistic simulations.
This approach leverages your validated hits to model expected performance:

```{r}
# First, identify proteins from your results to use as templates
# Run simulation using your data as templates

# concentrations must be a subset of c(0, 1, 3, 10, 30, 100, 300, 1000, 3000).
custom_simulation <- futureExperimentSimulation(
  ...
  strong_proteins = 'YBR117C',                # User defined strong responder
  weak_proteins = 'YNR054C',                    # User defined weak responder 
  no_interaction_proteins = 'Q0250',          # User defined negative control 
  drug_name = "T",                     # Specify which drug to model
...
1.  Szvetecz, S., et al. (2025).MSstatsResponse: Semi-parametric statistical model enhances drug-protein interaction detection and IC50 estimation in chemoproteomics experiments.
    *Manuscript in preparation.*

2.  Choi, M., et al. (2014).
    MSstats: an R package for statistical analysis of quantitative mass spectrometry-based proteomic experiments.
    *Bioinformatics*, 30(17), 2524-2526.
